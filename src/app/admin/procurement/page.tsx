                ...
                {purchaseOrders.map((po) => (
                  <tr key={po.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-3.5">
                      ...
                      {po.payment_method && (
                        <span className={`inline-block mt-1 text-[9px] font-extrabold px-1.5 py-0.5 rounded ${
                          po.payment_method === 'TUNAI' ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-blue-100 text-blue-800 border border-blue-200'
                        }`}>
                          {po.payment_method}
                        </span>
                      )}
                    ...
                    <td className="px-6 py-3.5 font-semibold text-slate-700">{po.distributor_name}</td>

                    <td className="px-6 py-3.5 text-xs text-slate-600">
                      {po.items.map((item, idx) => (
                        <div key={idx}>• {item.product_name} (<span className="font-mono text-emerald-700">{formatKg(item.qty_ordered_kg)}</span>)</div>
                      ))}
                    </td>

                    {showFinancialColumn && (
                      <td className="px-6 py-3.5 font-mono font-bold text-slate-800">{formatIDR(po.total_amount)}</td>
                    )}
